...
class CloudDB:
    ...
    def _request(cls, method, endpoint, data=None):
        ...
        try:
            ...
            elif method == "PATCH": r = requests.patch(url, headers=headers, json=data)
    ...
    def activate_key_cloud(cls, key_text, used_by, used_hwid):
        return cls._request("PATCH", f"license_keys?key_text=eq.{key_text}", {"used_by": used_by, "used_hwid": used_hwid, "used_at": datetime.now().isoformat(), "is_used": 1})
...
class ComputerID:
    ...
    def get_full_hwid():
        ...
        return hashlib.sha512(encrypt(data)).hexdigest()[:64]
...
class UserDB:
    ...
    def create_tables(self):
        self.cursor.execute('''CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT NOT NULL, hwid TEXT UNIQUE NOT NULL, mac TEXT, computer_name TEXT, is_owner INTEGER DEFAULT 0, is_admin INTEGER DEFAULT 0, is_banned INTEGER DEFAULT 0, created_at TEXT, expires_at TEXT, last_active TEXT, saved_key TEXT)''')
        self.cursor.execute('''CREATE TABLE IF NOT EXISTS license_keys (id INTEGER PRIMARY KEY AUTOINCREMENT, key_text TEXT UNIQUE NOT NULL, created_at TEXT, expires_at TEXT, used_by TEXT, used_hwid TEXT, used_at TEXT, is_used INTEGER DEFAULT 0, owner_hwid TEXT)''')
    ...
    def check_master_key(self, key): return key.upper() == MASTER_KEY.upper()
    def save_license(self, key):
        with open(LICENSE_FILE, 'w') as f: f.write(base64.b64encode(encrypt(key)).decode('utf-8'))
    def load_license(self):
        if os.path.exists(LICENSE_FILE):
            try:
                with open(LICENSE_FILE, 'r') as f: return decrypt(base64.b64decode(f.read().strip().encode('utf-8')))
            except: return None
        return None
    ...
    def generate_key(self, months=1, custom_key=None):
        ...
        try:
            self.cursor.execute('INSERT INTO license_keys (key_text, created_at, expires_at, is_used, owner_hwid) VALUES (?, ?, ?, 0, ?)', (encrypt(key), encrypt(created_at), encrypt(expires_at), encrypt(owner_hwid)))
            self.conn.commit()
        except sqlite3.IntegrityError: return False, None
        try: CloudDB.insert_key(key, expires_at, owner_hwid)
        except: pass
        return True, key
    ...
    def activate_key(self, key_text, save=True):
        ...
        if self.check_master_key(key_upper):
            ...
            if user:
                self.cursor.execute('UPDATE users SET username = ?, is_owner = 1, is_admin = 1, is_banned = 0, expires_at = ?, saved_key = ? WHERE hwid = ?', (encrypted_username, encrypted_expires, encrypted_key, encrypted_hwid))
            else:
        ...
        if owner_hwid and owner_hwid != hwid: return False, "❌ ЭТОТ КЛЮЧ НЕ ДЛЯ ТВОЕГО КОМПЬЮТЕРА!"
        if is_used and used_hwid and used_hwid != hwid: return False, "❌ КЛЮЧ УЖЕ АКТИВИРОВАН НА ДРУГОМ КОМПЬЮТЕРЕ!"
        ...
        expiry = datetime.fromisoformat(expires_at)
        if datetime.now() > expiry: return False, f"❌ КЛЮЧ ИСТЕК {expiry.strftime('%d.%m.%Y')}!"
        self.cursor.execute('UPDATE license_keys SET used_by = ?, used_hwid = ?, used_at = ?, is_used = 1 WHERE key_text = ?', (encrypt(username), encrypt(hwid), encrypt(datetime.now().isoformat()), encrypted_key))
        user = self.cursor.execute('SELECT * FROM users WHERE hwid = ?', (encrypt(hwid),)).fetchone()
        if user:
            self.cursor.execute('UPDATE users SET username = ?, expires_at = ?, is_banned = 0, saved_key = ? WHERE hwid = ?', (encrypt(username), expires_enc, encrypted_key, encrypt(hwid)))
        else:
    ...
    def check_access(self):
        hwid = self.get_hwid()
        encrypted_hwid = encrypt(hwid)
        result = self.cursor.execute('SELECT username, is_owner, is_admin, is_banned, expires_at FROM users WHERE hwid = ?', (encrypted_hwid,)).fetchone()
        ...
        self.cursor.execute('UPDATE users SET last_active = ? WHERE hwid = ?', (encrypt(datetime.now().isoformat()), encrypted_hwid))
        self.conn.commit()
    ...
    def get_all_users(self):
        ...
        for user in users:
            ...
            decrypted_users.append((decrypt(username_enc), is_owner, is_admin, is_banned, decrypt(expires_enc), decrypt(hwid_enc), decrypt(saved_key_enc) if saved_key_enc else None))
        return decrypted_users
    def get_all_keys(self):
        ...
        for key in keys:
            ...
            decrypted_keys.append((decrypt(key_enc), decrypt(created_enc), decrypt(expires_enc), decrypt(used_by_enc) if used_by_enc else None, decrypt(used_hwid_enc) if used_hwid_enc else None, is_used, decrypt(owner_hwid_enc) if owner_hwid_enc else None))
        return decrypted_keys
    def give_access(self, username, months=1):
        ...
        if user:
            self.cursor.execute('UPDATE users SET expires_at = ?, is_banned = 0 WHERE username = ?', (encrypted_expires, encrypted_username))
        else:
            self.cursor.execute('INSERT INTO users (username, hwid, created_at, expires_at) VALUES (?, ?, ?, ?)', (encrypted_username, encrypt(f"MANUAL_{uuid.uuid4().hex[:8]}"), encrypt(datetime.now().isoformat()), encrypted_expires))
        self.conn.commit()
    ...
    def extend_access(self, username, months=1):
        ...
        if result:
            ...
            self.cursor.execute('UPDATE users SET expires_at = ? WHERE username = ?', (encrypt(new_expiry.isoformat()), encrypt(username)))
            self.conn.commit()
...
class StarBackground:
    def __init__(self, canvas, num_stars=150):
        ...
        for _ in range(num_stars):
            ...
            self.stars.append({'x': x, 'y': y, 'size': size, 'speed': speed, 'brightness': brightness, 'phase': phase, 'dx': dx, 'dy': dy, 'color': color_choice})
    def update(self):
        ...
        for star in self.stars:
            ...
            colors = {'blue': f"#{min(255, b):02x}{min(255, b//3):02x}{min(255, b):02x}", 'white': f"#{min(255, b):02x}{min(255, b):02x}{min(255, b):02x}", 'gold': f"#{min(255, b):02x}{min(255, b//2):02x}{min(255, b//4):02x}", 'pink': f"#{min(255, b):02x}{min(255, b//3):02x}{min(255, b//2):02x}"}
            color = colors.get(star['color'], f"#{min(255, b):02x}{min(255, b//2):02x}{min(255, b):02x}")
            s = star['size']
            glow = s * 2
            self.canvas.create_oval(star['x'] - glow, star['y'] - glow, star['x'] + glow, star['y'] + glow, fill='', outline=color, width=0.5, tags="star", stipple="gray50")
            self.canvas.create_oval(star['x'] - s, star['y'] - s, star['x'] + s, star['y'] + s, fill=color, outline='', tags="star")
...
class ActivationWindow:
    def __init__(self):
        self.window = tk.Tk()
        self.window.title(f"🔐 АКТИВАЦИЯ | AWESOMETROLLING")
        ...
        self.canvas = tk.Canvas(self.window, width=600, height=650, bg='#0a0e27', highlightthickness=0)
        self.canvas.pack(fill=tk.BOTH, expand=True)
        self.stars = StarBackground(self.canvas, 100)
        self.stars.update()
        shadow = tk.Frame(self.canvas, bg='#1a1f4a', width=580, height=600)
        shadow.place(x=10, y=25)
        main_frame = tk.Frame(self.canvas, bg='#111638', width=580, height=600)
        main_frame.place(x=10, y=25)
        grad = tk.Frame(main_frame, bg='#6c5ce7', height=4)
        grad.pack(fill=tk.X, padx=0, pady=0)
        header = tk.Frame(main_frame, bg='#111638')
        header.pack(fill=tk.X, padx=30, pady=(20,5))
        tk.Label(header, text="🔥 AWESOMETROLLING", font=("Segoe UI", 26, "bold"), bg='#111638', fg='#ffd700').pack()
        tk.Label(header, text="🔐 АКТИВАЦИЯ ПРОГРАММЫ", font=("Segoe UI", 12), bg='#111638', fg='#dfe6e9').pack()
        info_frame = tk.Frame(main_frame, bg='#1a1f4a')
        info_frame.pack(pady=10, padx=30, fill=tk.X)
        info_frame.config(height=80)
        info_frame.pack_propagate(False)
        info_inner = tk.Frame(info_frame, bg='#1a1f4a')
        info_inner.pack(fill=tk.BOTH, padx=15, pady=10)
        tk.Label(info_inner, text=f"💻 Компьютер: {ComputerID.get_username()}", bg='#1a1f4a', fg='#dfe6e9', font=("Segoe UI", 11)).pack(anchor='w')
        tk.Label(info_inner, text=f"🆔 HWID: {ComputerID.get_full_hwid()[:24]}...", bg='#1a1f4a', fg='#b2bec3', font=("Segoe UI", 9)).pack(anchor='w')
        center_frame = tk.Frame(main_frame, bg='#111638')
        center_frame.pack(pady=15, padx=30, fill=tk.BOTH, expand=True)
        tk.Label(center_frame, text="⚡ КУПИ ДОСТУП ⚡", font=("Segoe UI", 20, "bold"), bg='#111638', fg='#ff6b35').pack()
        tk.Label(center_frame, text="У ВЛАДЕЛЬЦА", font=("Segoe UI", 12), bg='#111638', fg='#dfe6e9').pack()
        contact_frame = tk.Frame(center_frame, bg='#222860')
        contact_frame.pack(pady=8, padx=20, fill=tk.X)
        contact_frame.config(height=50)
        contact_frame.pack_propagate(False)
        contact_inner = tk.Frame(contact_frame, bg='#222860')
        contact_inner.pack(fill=tk.BOTH, padx=10, pady=5)
        tk.Label(contact_inner, text="🔥 @flidges 🔥", font=("Segoe UI", 16, "bold"), bg='#222860', fg='#ffd700').pack(side=tk.LEFT)
        tk.Label(contact_inner, text="📩 Telegram", font=("Segoe UI", 10), bg='#222860', fg='#4fc3f7').pack(side=tk.RIGHT)
        tk.Label(center_frame, text="💰 Цена - узнайте у @flidges", font=("Segoe UI", 12, "bold"), bg='#111638', fg='#00ff88').pack(pady=5)
        sep = tk.Frame(center_frame, bg='#636e72', height=1, width=300)
        sep.pack(pady=10)
        key_frame = tk.Frame(center_frame, bg='#111638')
        key_frame.pack(pady=10, fill=tk.X)
        tk.Label(key_frame, text="Или введите ключ активации:", bg='#111638', fg='#b2bec3', font=("Segoe UI", 10)).pack(anchor='w')
        entry_frame = tk.Frame(key_frame, bg='#111638')
        entry_frame.pack(fill=tk.X, pady=5)
        self.key_entry = tk.Entry(entry_frame, bg='#1a1f4a', fg='#00ff88', font=("Segoe UI", 14), relief=tk.FLAT, borderwidth=2, insertbackground='#dfe6e9')
        self.key_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0,10))
        self.key_entry.bind('<Return>', lambda e: self.activate())
        self.activate_btn = tk.Button(entry_frame, text="✅ АКТИВИРОВАТЬ", command=self.activate, bg='#6c5ce7', fg='white', font=("Segoe UI", 10, "bold"), relief=tk.FLAT, cursor="hand2", padx=15, pady=8)
        self.activate_btn.pack(side=tk.RIGHT)
        self.status_frame = tk.Frame(center_frame, bg='#111638', height=50)
        self.status_frame.pack(fill=tk.X, pady=5)
        self.status_frame.pack_propagate(False)
        self.status_label = tk.Label(self.status_frame, text="", bg='#111638', fg='#e17055', font=("Segoe UI", 11, "bold"))
        self.status_label.pack(fill=tk.BOTH, expand=True)
        footer = tk.Frame(main_frame, bg='#111638')
        footer.pack(side=tk.BOTTOM, fill=tk.X, pady=10)
        tk.Label(footer, text="© 2026 @flidges | Версия 3.0", bg='#111638', fg='#636e72', font=("Segoe UI", 8)).pack()
        self.window.mainloop()
    def activate(self):
        key = self.key_entry.get().strip()
        if not key:
            self.status_label.config(text="❌ ВВЕДИТЕ КЛЮЧ!", fg='#e17055')
            return
        success, msg = db.activate_key(key)
        if success:
            self.status_label.config(text="✅ " + msg, fg='#00b894')
            self.activate_btn.config(bg='#00b894', text="✅ АКТИВИРОВАНО!")
            self.window.after(1500, self.close_and_start)
        else:
            self.status_label.config(text="❌ " + msg, fg='#e17055')
    def close_and_start(self):
        if self.stars: self.stars.stop()
        self.window.destroy()
        start_program()
...
def start_program():
    hide_console()
    root = tk.Tk()
    root.title(f"🔥 AWESOMETROLLING | @flidges")
...
class InsultApp:
    def __init__(self, root):
        ...
        app_instance = self
        self.root = root
        self.root.title(f"🔥 AWESOMETROLLING | @flidges")
        ...
        if saved_key and db.check_master_key(saved_key):
            ...
            user = db.cursor.execute('SELECT is_admin, is_owner FROM users WHERE hwid = ?', (encrypt(hwid),)).fetchone()
            if user and (user[0] == 1 or user[1] == 1):
                self.is_admin = True
        self.canvas = tk.Canvas(self.root, bg='#0a0e27', highlightthickness=0)
        self.canvas.pack(fill=tk.BOTH, expand=True)
        self.stars = StarBackground(self.canvas, 150)
        self.stars.update()
        main_frame = tk.Frame(self.canvas, bg='#111638', bd=2, relief=tk.FLAT)
        main_frame.place(relx=0.5, rely=0.5, anchor=tk.CENTER, width=860, height=660)
        self.main_frame = main_frame
        title_frame = tk.Frame(main_frame, bg='#111638', height=80)
        title_frame.pack(fill=tk.X, padx=0, pady=0)
        title_frame.pack_propagate(False)
        title_inner = tk.Frame(title_frame, bg='#111638')
        title_inner.pack(fill=tk.BOTH, padx=20, pady=10)
        tk.Label(title_inner, text="🔥 AWESOMETROLLING", font=("Segoe UI", 26, "bold"), bg='#111638', fg='#ffd700').pack(side=tk.LEFT)
        tk.Label(title_inner, text="✨ Создатель: awesome / tg @flidges ✨", font=("Segoe UI", 10), bg='#111638', fg='#ff6b35').pack(side=tk.RIGHT)
        top_frame = tk.Frame(main_frame, bg='#0a0e27')
        top_frame.pack(fill=tk.X, padx=0, pady=5)
        self.admin_btn = GlowButton(top_frame, text="⚙️ АДМИН-ПАНЕЛЬ (F6)", command=self.toggle_admin, bg='#6c5ce7', fg='#dfe6e9', font=("Segoe UI", 10, "bold"), padx=14, pady=5)
        self.admin_btn.pack(side=tk.LEFT, padx=5)
        self.fs_btn = GlowButton(top_frame, text="⛶ ПОЛНЫЙ ЭКРАН (F11)", command=self.toggle_fullscreen, bg='#222860', fg='#dfe6e9', font=("Segoe UI", 10, "bold"), padx=14, pady=5)
        self.fs_btn.pack(side=tk.RIGHT, padx=5)
        self.logout_btn = GlowButton(top_frame, text="🚪 ВЫЙТИ (F9)", command=self.logout, bg='#e17055', fg='#dfe6e9', font=("Segoe UI", 10, "bold"), padx=14, pady=5)
        self.logout_btn.pack(side=tk.RIGHT, padx=5)
        stats_frame = tk.Frame(main_frame, bg='#0a0e27')
        stats_frame.pack(pady=5)
        self.status_label = tk.Label(stats_frame, text="⏸️ Ожидание...", bg='#0a0e27', fg='#fdcb6e', font=("Segoe UI", 13, "bold"))
        self.status_label.pack(side=tk.LEFT, padx=10)
        self.count_label = tk.Label(stats_frame, text="📨 0", bg='#0a0e27', fg='#00ff88', font=("Segoe UI", 13, "bold"))
        self.count_label.pack(side=tk.LEFT, padx=10)
        self.preview = scrolledtext.ScrolledText(main_frame, height=8, bg='#1a1f4a', fg='#dfe6e9', insertbackground='white', font=("Segoe UI", 10), relief=tk.FLAT, borderwidth=2, padx=15, pady=15)
        self.preview.pack(padx=10, pady=5, fill=tk.BOTH, expand=True)
        self.preview.insert("1.0", f"""🔥 AWESOMETROLLING
...
✅ Автовход по ключу
✅ Создатель: awesome / tg @flidges
✅ Сделано с любовью и матом""")
        self.preview.config(state=tk.DISABLED)
        btn_frame = tk.Frame(main_frame, bg='#0a0e27')
        btn_frame.pack(pady=8)
        self.start_btn = GlowButton(btn_frame, text="🤖 СТАРТ (F3)", command=self.start_spam, bg='#00b894', fg='#dfe6e9', font=("Segoe UI", 10, "bold"), width=16, padx=5, pady=8)
        self.start_btn.pack(side=tk.LEFT, padx=5)
        self.stop_btn = GlowButton(btn_frame, text="🛑 СТОП (F4)", command=self.stop_spam, bg='#e17055', fg='#dfe6e9', font=("Segoe UI", 10, "bold"), width=16, padx=5, pady=8)
        self.stop_btn.pack(side=tk.LEFT, padx=5)
        self.pause_btn = GlowButton(btn_frame, text="⏸️ ПАУЗА (F5)", command=self.toggle_pause, bg='#6c5ce7', fg='#dfe6e9', font=("Segoe UI", 10, "bold"), width=16, padx=5, pady=8)
        self.pause_btn.pack(side=tk.LEFT, padx=5)
        bottom_frame = tk.Frame(main_frame, bg='#0a0e27')
        bottom_frame.pack(pady=5)
        tk.Label(bottom_frame, text="F3-СТАРТ | F4-СТОП | F5-ПАУЗА | F6-АДМИН | F9-ВЫХОД | F11-ПОЛНЫЙ ЭКРАН", bg='#0a0e27', fg='#b2bec3', font=("Segoe UI", 9)).pack()
        tk.Label(bottom_frame, text="💜 Сделано с любовью и матом 💜", bg='#0a0e27', fg='#fd79a8', font=("Segoe UI", 10, "bold")).pack()
        self.admin_panel = AdminPanel(self.root, self.is_admin)
        self.fullscreen = False
        self.setup_hotkeys()
        self.update_stats()
    def toggle_admin(self): self.admin_panel.toggle()
    def toggle_fullscreen(self):
        self.fullscreen = not self.fullscreen
        self.root.attributes('-fullscreen', self.fullscreen)
        if self.fullscreen:
            self.fs_btn.config(text="⛶ ОКОННЫЙ РЕЖИМ (F11)", bg='#fdcb6e')
        else:
            self.fs_btn.config(text="⛶ ПОЛНЫЙ ЭКРАН (F11)", bg='#222860')
    ...
    def update_counters(self):
        try: self.count_label.config(text=f"📨 {message_count}")
        except: pass
    def update_ui_state(self):
        try:
            if is_paused:
                self.status_label.config(text="⏸️ ПАУЗА", fg='#fdcb6e')
                self.pause_btn.config(text="▶️ ВОЗОБНОВИТЬ (F5)", bg='#fdcb6e')
            elif not stop_spam and spam_thread and spam_thread.is_alive():
                self.status_label.config(text="🧠 ГЕНЕРАЦИЯ", fg='#00b894')
                self.start_btn.config(bg='#222860', text="🧠 РАБОТАЕТ...")
                self.pause_btn.config(text="⏸️ ПАУЗА (F5)", bg='#6c5ce7')
            else:
                self.status_label.config(text="⏸️ Остановлено", fg='#b2bec3')
                self.start_btn.config(bg='#00b894', text="🤖 СТАРТ (F3)")
                self.pause_btn.config(text="⏸️ ПАУЗА (F5)", bg='#6c5ce7')
        except: pass
...
class AdminPanel:
    ...
    def create_panel(self):
        ...
        self.window.title("✨ АДМИН-ПАНЕЛЬ | AWESOMETROLLING")
        ...
        y = (self.window.winfo_screenheight() // 2) - (height // 2)
        self.window.geometry(f'{width}x{height}+{x}+{y}')
        canvas = tk.Canvas(self.window, bg='#0a0e27', highlightthickness=0)
        canvas.pack(fill=tk.BOTH, expand=True)
        self.admin_stars = StarBackground(canvas, 100)
        self.admin_stars.update()
        main_frame = tk.Frame(canvas, bg='#111638', bd=2, relief=tk.FLAT)
        main_frame.place(relx=0.5, rely=0.5, anchor=tk.CENTER, width=910, height=710)
        title_frame = tk.Frame(main_frame, bg='#111638', height=60)
        title_frame.pack(fill=tk.X, padx=0, pady=0)
        title_frame.pack_propagate(False)
        title_inner = tk.Frame(title_frame, bg='#111638')
        title_inner.pack(fill=tk.BOTH, padx=20, pady=10)
        tk.Label(title_inner, text="✨ АДМИН-ПАНЕЛЬ", font=("Segoe UI", 22, "bold"), bg='#111638', fg='#ffd700').pack(side=tk.LEFT)
        tk.Label(title_inner, text="⭐ @flidges", font=("Segoe UI", 12), bg='#111638', fg='#ff6b35').pack(side=tk.RIGHT)
        sep = tk.Frame(main_frame, bg='#00ff88', height=3)
        ...
        style.configure('TNotebook.Tab', background='#111638', foreground='#dfe6e9', padding=[20, 8], font=("Segoe UI", 10, "bold"))
        style.map('TNotebook.Tab', background=[('selected', '#6c5ce7')])
        self.tab_main = tk.Frame(self.notebook, bg='#0a0e27')
        self.notebook.add(self.tab_main, text="📊 Главная")
        self.create_main_tab()
        self.tab_about = tk.Frame(self.notebook, bg='#0a0e27')
        self.notebook.add(self.tab_about, text="💜 О нас")
        self.create_about_tab()
        if self.is_admin:
            self.tab_users = tk.Frame(self.notebook, bg='#0a0e27')
            self.notebook.add(self.tab_users, text="👥 Пользователи")
            self.create_users_tab()
            self.tab_keys = tk.Frame(self.notebook, bg='#0a0e27')
            self.notebook.add(self.tab_keys, text="🔑 Ключи")
            self.create_keys_tab()
            self.tab_stats = tk.Frame(self.notebook, bg='#0a0e27')
            self.notebook.add(self.tab_stats, text="📈 Статистика")
            self.create_stats_tab()
        self.update_stats()
    def create_main_tab(self):
        tab = self.tab_main
        speed_frame = tk.Frame(tab, bg='#0a0e27')
        speed_frame.pack(pady=10, padx=20, fill=tk.X)
        tk.Label(speed_frame, text="🚀 Скорость отправки", font=("Segoe UI", 14, "bold"), bg='#0a0e27', fg='#a29bfe').pack(anchor='w')
        speed_control = tk.Frame(speed_frame, bg='#0a0e27')
        speed_control.pack(fill=tk.X, pady=5)
        self.speed_slider = tk.Scale(speed_control, from_=0.001, to=0.45, resolution=0.001, orient=tk.HORIZONTAL, length=500, bg='#0a0e27', fg='#dfe6e9', troughcolor='#1a1f4a', sliderlength=22, highlightthickness=0)
        self.speed_slider.set(spam_speed)
        self.speed_slider.pack(side=tk.LEFT, fill=tk.X, expand=True)
        self.speed_label = tk.Label(speed_control, text=f"{spam_speed:.3f}с", bg='#0a0e27', fg='#ffd700', font=("Segoe UI", 18, "bold"), width=8)
        self.speed_label.pack(side=tk.LEFT, padx=10)
        def update_speed(val):
            val = float(val)
            self.speed_label.config(text=f"{val:.3f}с")
            if self.speed_update_timer:
                self.window.after_cancel(self.speed_update_timer)
            def apply_speed():
                global spam_speed
                spam_speed = val
                settings['spam_speed'] = val
                save_settings(settings)
            self.speed_update_timer = self.window.after(300, apply_speed)
        self.speed_slider.config(command=update_speed)
        preset_frame = tk.Frame(tab, bg='#0a0e27')
        preset_frame.pack(pady=5, padx=20, fill=tk.X)
        tk.Label(preset_frame, text="⚡ Быстрые пресеты", font=("Segoe UI", 11, "bold"), bg='#0a0e27', fg='#b2bec3').pack(anchor='w')
        preset_btns = tk.Frame(preset_frame, bg='#0a0e27')
        preset_btns.pack(fill=tk.X, pady=5)
        for name, speed in [("🐢 0.1с", 0.1), ("🚶 0.05с", 0.05), ("🏃 0.02с", 0.02), ("🚀 0.005с", 0.005), ("🔥 0.001с", 0.001)]:
            btn = GlowButton(preset_btns, text=name, command=lambda sp=speed: self.apply_preset(sp), bg='#222860', fg='#dfe6e9', font=("Segoe UI", 9, "bold"), padx=14, pady=6)
            btn.pack(side=tk.LEFT, padx=3)
        info_frame = tk.Frame(tab, bg='#0a0e27')
        info_frame.pack(pady=15, padx=20, fill=tk.BOTH, expand=True)
        tk.Label(info_frame, text="📊 Живая статистика", font=("Segoe UI", 14, "bold"), bg='#0a0e27', fg='#00ff88').pack(anchor='w')
        self.info_text = tk.Text(info_frame, height=8, bg='#111638', fg='#dfe6e9', font=("Consolas", 10), relief=tk.FLAT, borderwidth=2, padx=15, pady=12)
        self.info_text.pack(fill=tk.BOTH, expand=True, pady=5)
        self.info_text.insert("1.0", "⏳ Ожидание запуска...")
        self.info_text.config(state=tk.DISABLED)
        btn_frame = tk.Frame(tab, bg='#0a0e27')
        btn_frame.pack(pady=10)
        for text, cmd in [("🔄 Обновить", self.update_info), ("🧹 Сбросить счётчик", self.reset_counters)]:
            btn = GlowButton(btn_frame, text=text, command=cmd, bg='#1a1f4a', fg='#dfe6e9', font=("Segoe UI", 10, "bold"), padx=18, pady=6)
            btn.pack(side=tk.LEFT, padx=5)
    def create_about_tab(self):
        tab = self.tab_about
        about_frame = tk.Frame(tab, bg='#0a0e27')
        about_frame.pack(fill=tk.BOTH, expand=True, padx=40, pady=40)
        tk.Label(about_frame, text="🔥", font=("Segoe UI", 70), bg='#0a0e27').pack(pady=5)
        tk.Label(about_frame, text="AWESOMETROLLING", font=("Segoe UI", 26, "bold"), bg='#0a0e27', fg='#ffd700').pack(pady=5)
        tk.Label(about_frame, text="✨ Версия 3.0 ✨", font=("Segoe UI", 14), bg='#0a0e27', fg='#b2bec3').pack(pady=5)
        sep = tk.Frame(about_frame, bg='#00ff88', height=2, width=350)
        sep.pack(pady=15)
        tk.Label(about_frame, text="👨‍💻 РАЗРАБОТЧИК", font=("Segoe UI", 13, "bold"), bg='#0a0e27', fg='#dfe6e9').pack()
        tk.Label(about_frame, text="@flidges", font=("Segoe UI", 20, "bold"), bg='#0a0e27', fg='#fd79a8').pack(pady=3)
        tk.Label(about_frame, text="✨ Создатель: awesome / tg @flidges ✨", font=("Segoe UI", 12), bg='#0a0e27', fg='#ffd700').pack(pady=5)
        tk.Label(about_frame, text="💰 Цена - узнайте у @flidges", font=("Segoe UI", 12, "bold"), bg='#0a0e27', fg='#00ff88').pack(pady=5)
        sep2 = tk.Frame(about_frame, bg='#6c5ce7', height=1, width=250)
        sep2.pack(pady=10)
        for feat in ["🔥 Каждое сообщение уникально", "💎 Длинные связные предложения", "📚 60+ шаблонов", "⚡ Работает при свёрнутом окне", "🔒 Защита HWID", "💾 Автосохранение ключа"]:
            tk.Label(about_frame, text=feat, font=("Segoe UI", 11), bg='#0a0e27', fg='#00ff88').pack(pady=2)
        sep3 = tk.Frame(about_frame, bg='#6c5ce7', height=1, width=200)
        sep3.pack(pady=10)
        tk.Label(about_frame, text="💜 Сделано с любовью и матом 💜", font=("Segoe UI", 14, "bold"), bg='#0a0e27', fg='#fd79a8').pack(pady=5)
        tk.Label(about_frame, text="© 2026 Все права защищены 🚀", font=("Segoe UI", 9), bg='#0a0e27', fg='#636e72').pack(pady=5)
    def create_users_tab(self):
        if not self.is_admin: return
        tab = self.tab_users
        control_frame = tk.Frame(tab, bg='#0a0e27')
        control_frame.pack(pady=10, padx=20, fill=tk.X)
        tk.Label(control_frame, text="👥 Управление пользователями", font=("Segoe UI", 14, "bold"), bg='#0a0e27', fg='#ffd700').pack(anchor='w')
        input_frame = tk.Frame(control_frame, bg='#0a0e27')
        input_frame.pack(fill=tk.X, pady=5)
        self.user_entry = tk.Entry(input_frame, bg='#1a1f4a', fg='#dfe6e9', font=("Segoe UI", 11), relief=tk.FLAT, borderwidth=2, width=20)
        ...
        tk.Label(input_frame, text="мес.", bg='#0a0e27', fg='#b2bec3').pack(side=tk.LEFT)
        tk.Button(input_frame, text="✅ ВЫДАТЬ", command=lambda: self.give_access(months_var.get()), bg='#00b894', fg='white', font=("Segoe UI", 9, "bold"), relief=tk.FLAT, cursor="hand2", padx=10, pady=5).pack(side=tk.LEFT, padx=5)
        tk.Button(input_frame, text="🚫 ЗАБРАТЬ", command=self.revoke_access, bg='#e17055', fg='white', font=("Segoe UI", 9, "bold"), relief=tk.FLAT, cursor="hand2", padx=10, pady=5).pack(side=tk.LEFT, padx=5)
        tk.Button(input_frame, text="🔄 ПРОДЛИТЬ", command=lambda: self.extend_access(months_var.get()), bg='#6c5ce7', fg='white', font=("Segoe UI", 9, "bold"), relief=tk.FLAT, cursor="hand2", padx=10, pady=5).pack(side=tk.LEFT, padx=5)
        list_frame = tk.Frame(tab, bg='#0a0e27')
        ...
        tk.Label(tab, text="💡 Двойной клик по пользователю → бан/разбан", bg='#0a0e27', fg='#b2bec3', font=("Segoe UI", 9)).pack(pady=5)
        self.refresh_users()
    def create_keys_tab(self):
        ...
        control_frame = tk.Frame(tab, bg='#0a0e27')
        control_frame.pack(pady=10, padx=20, fill=tk.X)
        tk.Label(control_frame, text="🔑 Управление ключами", font=("Segoe UI", 14, "bold"), bg='#0a0e27', fg='#ffd700').pack(anchor='w')
        add_frame = tk.Frame(control_frame, bg='#0a0e27')
        add_frame.pack(fill=tk.X, pady=5)
        tk.Label(add_frame, text="Ключ:", bg='#0a0e27', fg='#dfe6e9', font=("Segoe UI", 10)).pack(side=tk.LEFT, padx=5)
        self.key_entry = tk.Entry(add_frame, bg='#1a1f4a', fg='#dfe6e9', font=("Segoe UI", 11), relief=tk.FLAT, borderwidth=2, width=20)
        ...
        tk.Label(add_frame, text="мес:", bg='#0a0e27', fg='#dfe6e9', font=("Segoe UI", 10)).pack(side=tk.LEFT, padx=5)
        ...
        tk.Button(add_frame, text="➕ ДОБАВИТЬ КЛЮЧ", command=self.add_key, bg='#00b894', fg='white', font=("Segoe UI", 9, "bold"), relief=tk.FLAT, cursor="hand2", padx=10, pady=5).pack(side=tk.LEFT, padx=5)
        tk.Button(add_frame, text="🎲 СГЕНЕРИРОВАТЬ", command=self.generate_key, bg='#6c5ce7', fg='white', font=("Segoe UI", 9, "bold"), relief=tk.FLAT, cursor="hand2", padx=10, pady=5).pack(side=tk.LEFT, padx=5)
        list_frame = tk.Frame(tab, bg='#0a0e27')
        list_frame.pack(pady=10, padx=20, fill=tk.BOTH, expand=True)
        ...
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        del_frame = tk.Frame(tab, bg='#0a0e27')
        del_frame.pack(pady=5, padx=20, fill=tk.X)
        tk.Button(del_frame, text="🗑 УДАЛИТЬ ВЫБРАННЫЙ КЛЮЧ", command=self.delete_key, bg='#e17055', fg='white', font=("Segoe UI", 9, "bold"), relief=tk.FLAT, cursor="hand2", padx=10, pady=5).pack(side=tk.LEFT)
        tk.Label(del_frame, text="💡 Выберите ключ в списке и нажмите УДАЛИТЬ", bg='#0a0e27', fg='#b2bec3', font=("Segoe UI", 9)).pack(side=tk.LEFT, padx=10)
        self.refresh_keys()
    def create_stats_tab(self):
        if not self.is_admin: return
        tab = self.tab_stats
        stats_frame = tk.Frame(tab, bg='#0a0e27')
        stats_frame.pack(fill=tk.BOTH, expand=True, padx=20, pady=20)
        tk.Label(stats_frame, text="📈 ДЕТАЛЬНАЯ СТАТИСТИКА", font=("Segoe UI", 18, "bold"), bg='#0a0e27', fg='#ffd700').pack(pady=10)
        self.stats_text = tk.Text(stats_frame, height=14, bg='#111638', fg='#dfe6e9', font=("Consolas", 11), relief=tk.FLAT, borderwidth=2, padx=20, pady=15)
        self.stats_text.pack(fill=tk.BOTH, expand=True, pady=10)
        self.stats_text.config(state=tk.DISABLED)
        btn_frame = tk.Frame(stats_frame, bg='#0a0e27')
        btn_frame.pack(pady=10)
        tk.Button(btn_frame, text="🔄 ОБНОВИТЬ", command=self.update_stats_display, bg='#6c5ce7', fg='white', font=("Segoe UI", 10, "bold"), relief=tk.FLAT, cursor="hand2", padx=20, pady=8).pack()
    ...
    def refresh_keys(self):
        if not self.is_admin: return
        for item in self.keys_tree.get_children(): self.keys_tree.delete(item)
            ...
            with open(SETTINGS_FILE, 'r', encoding='utf-8') as f:
                ...
                for key, value in encrypted_data.items():
                    if isinstance(value, str):
                        try: decrypted[key] = decrypt(value)
    ...
    try:
        ...
        for key, value in settings_data.items():
            if isinstance(value, (str, int, float, bool)):
                encrypted_data[key] = encrypt(str(value))
            else:
